...
function this.setWeather(weatherId)
    local wc = tes3.worldController.weatherController
    wc:switchImmediate(weatherId)
    wc:updateVisuals()
end

---@param volume number
function this.setMusicVolume(volume)
    tes3.worldController.audioController.volumeMusic = volume
end

---@param sceneNode niNode
---@return fun():niTimeController?
function this.controllersRecursive(sceneNode)
    return coroutine.wrap(function()
        for node in table.traverse({ sceneNode }) do
            local controller = node.controller --[[@as niTimeController?]]
            while controller do
                coroutine.yield(controller)
